...
export default function PanelContainer({ content }: { content: string }) {
  ...
  return (
    <div className="relative shrink-0 flex">
      {/* 悬浮触发区 + 半圆按钮（贴合竖线） */}
      <div className="absolute left-0 top-0 bottom-0 w-5 -translate-x-full z-20 group/toggle">
        <button
          onClick={() => setCollapsed(!collapsed)}
          className={`
            absolute right-0 top-1/2 -translate-y-1/2
            w-[18px] h-10 bg-[#F7F5F0] border border-[#E8E4DD] rounded-l-full
            flex items-center justify-center
            transition-all duration-500 ease-out
            opacity-0 group-hover/toggle:opacity-100
            ${collapsed ? 'translate-x-2 group-hover/toggle:translate-x-0' : 'translate-x-0'}
          `}
          aria-label={collapsed ? '展开单词栏' : '收起单词栏'}
        >
          <ChevronRight
            size={12}
            className={`text-[#78716C] transition-transform duration-500 ${
